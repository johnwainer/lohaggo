...
Investiga lo nuevo con tus herramientas y entrega el resultado con la herramienta entregar_analisis: la causa probable de cada cosa nueva, qué tan grave es y qué recomiendas. No repitas hallazgos que ya están abiertos salvo que hayan cambiado.`
}
...
Usa tendencias_negocio (${p.kind === 'daily' ? '7d' : '30d'}) y las demás herramientas que necesites. Escribe el informe para el superadmin con la herramienta entregar_informe: cómo va el negocio, qué pasó, qué quedó pendiente y qué recomiendas${p.kind === 'weekly' ? ', con tendencias y qué funcionó o no' : ''}.`
}
