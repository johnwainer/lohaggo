...
export default function AdminCommunicationsPage() {
  ...
  useEffect(() => {
    ...
    if (!campForm.partnerServiceIds.length && !campForm.partnerCategoryIds.length && campForm.partnerFilterMode === 'ALL' && !campForm.partnerWithoutDocs && !campForm.partnerWithoutStudies && !campForm.partnerWithoutServices && campForm.partnerOnlyActive && !campForm.partnerOnlyVerified) return
    setCampForm((prev) => ({ ...prev, partnerFilterMode: 'ALL', partnerCategoryIds: [], partnerServiceIds: [], partnerWithoutDocs: false, partnerWithoutStudies: false, partnerWithoutServices: false, partnerOnlyActive: true, partnerOnlyVerified: false }))
  }, [campForm.targetRole, campForm.partnerFilterMode, campForm.partnerCategoryIds.length, campForm.partnerServiceIds.length, campForm.partnerWithoutDocs, campForm.partnerWithoutStudies, campForm.partnerWithoutServices, campForm.partnerOnlyActive, campForm.partnerOnlyVerified])
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-4">
                      ...
                      <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={campForm.partnerOnlyVerified}
                          onChange={(e) => setCampForm((prev) => ({ ...prev, partnerOnlyVerified: e.target.checked }))}
                        />
                        <span>Solo socios verificados <span className="text-gray-400">(con documento de identidad aprobado, contratables)</span></span>
                      </label>
                      <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
                        <input
                          type="checkbox"
                      ...
                      <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={campForm.partnerWithoutServices}
                          onChange={(e) => setCampForm((prev) => ({ ...prev, partnerWithoutServices: e.target.checked }))}
                        />
                        <span>Sin servicios activos <span className="text-gray-400">(no han publicado ningún servicio)</span></span>
                      </label>
                    </div>
